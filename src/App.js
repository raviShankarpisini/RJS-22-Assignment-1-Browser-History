...
class App extends Component {
  ...
  deleteHistory = id => {
    ...
    const listAfterDelete = list.filter(eachHistory => eachHistory.id !== id)
    this.setState({list: listAfterDelete})
  }

  render() {
    const {searchInput, list} = this.state
    const searchHistoryResult = list.filter(eachHistory =>
    ...
    return (
      <div className="bg-container">
        ...
        <div className="history-bottom-container">
          <ul>
            {searchHistoryResult.map(eachHistory => (
              <HistoryComponent
          ...
          {list.length === 0 || searchHistoryResult.length === 0 ? (
